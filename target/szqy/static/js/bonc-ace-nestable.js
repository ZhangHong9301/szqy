...
(function ($) {
    ...
    BoncAceNestable.prototype = {
        ...
        render: function () {
        	...
        	$("<div></div>").addClass("dd dd-draghandle").attr("id","nestable").append(
        		$("<ol></ol>").addClass("dd-list").append(
        			$("<li></li>").addClass("dd-item dd2-item").attr("data-id","0").append(
        				...
        				$("<ol></ol>").addClass("dd-list").attr('id',this.data.root)
        			)		
        		)
        	...
        	this.genMenu(dataList, $('#'+this.data.root));
        ...
        genMenu: function(menuList, parent) {
        	for (var menu in menuList) {
        		if(menuList[menu].children!=undefined){
        		     var ol =  $("<ol></ol>").addClass("dd-list").appendTo(
        		    		   ...
        		    		   ).attr("disOrd",menuList[menu].disOrd).attr("isLeaf",menuList[menu].isLeaf).appendTo(parent));
       ...
       toolBox:function(node){
        ...
        setIndeterminate:function(obj,status,checkFunc){
        	...
        	if(parentOl.attr("id")== "root" || parentOl.attr("id")== "root1" || parentOl.attr("id")== "root2"){
        		return;
        	}
       ...
       nestableLoadData:function(){
        ...
        refreshNode:function(treeData){
        	...
        	thisObj=$("<li></li>").addClass("dd-item").attr("data-id",treeData.id).append(
        			...
        			).attr("disOrd",treeData.disOrd).attr("isLeaf",treeData.isLeaf);
        	//添加孩子
        	if(childeNodeOl!=undefined){
        ...
        getData:function(elm){
			...
			return {id:nodeId,type:nodeType,name:nodeText,disOrd:nodeOrd,isLeaf:isLeaf};
        },
